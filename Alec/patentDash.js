...
function setup() {
	...
	d3.json("data/cpcWithCountyAndCBSA.json", function(json,error) {
	console.log("got here");
	//d3.json("data/cpcWithCBSAOnly.json", function(json, error){
	//	data = json;
		console.log("got here too");
        cpcData = json.cpcData;
        console.log(cpcData[1]);
...
//	       data[p.cpc] = p;
             ...
             p.cbsaSum = +p.cbsaSum;
             p.cbsaNormalized = +p.cbsaNormalized;
            //p.sum = +p.sum;
            //p.normalized = +p.normalized
        	year = +p.year;
	       data[p.cpc][year][p.cbsaID] = p;
           }
        );
